...
def create_staging_version(law_version: str, notes: Optional[str] = None) -> dict:
    ...
    try:
        ...
        if not validation["passed"]:
            # BUG FIX: If DB is fresh/empty (no clauses yet), validation fails because
            # rights_clauses_map has stale entries from a previous reset.
            # An empty clause store is NOT a real validation failure — it just means
            # no documents have been ingested yet. Allow staging in this case.
            critical_errors = validation.get("errors", [])
            total_clauses   = validation.get("total_clauses", 0)

            # Only block if there are actual clauses with real integrity issues
            real_errors = [
                e for e in critical_errors
                if not (e.get("code") == "MAP_DEAD_CLAUSE" and total_clauses == 0)
            ]
